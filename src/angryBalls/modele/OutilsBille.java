...
public class OutilsBille {
	 ...
	 * 
	 * */
	public static Vector<Bille> autresBilles(Bille cetteBille,
			Vector<Bille> billes) {
	 ...
	 * @throws  
	 * */
	public static boolean gestionCollisionBilleBille(Bille cetteBille,
			Vector<Bille> billes) {
		// --- on récupère d'abord dans autresBilles toutes les billes sauf
		// cetteBille ----

		Vector<Bille> autresBilles = OutilsBille.autresBilles(cetteBille,
				billes);
		...
		for (i = 0; i < autresBilles.size(); ++i) {
			...
			if (Collisions.CollisionBilleBille(cetteBille.getPositionActuelle(),
					cetteBille.getRayon(), cetteBille.getVitesse(),
					cetteBille.masse(), billeCourante.getPositionActuelle(),
					billeCourante.getRayon(), billeCourante.getVitesse(),
					billeCourante.masse())) {
				return true;
			}
		}
	 ...
	 * 
	 * */
	public static Vecteur gestionAccélérationNewton(Bille cetteBille,
			Vector<Bille> billes) {
		...
		for (i = 0; i < d; ++i) {
			...
			C[i] = billeCourante.getPositionActuelle();
		}

		// ------------------ à présent on calcule le champ de gravité exercé
		// par les autres billes sur cette bille ------------------

		return MecaniquePoint.champGravitéGlobal(cetteBille.getPositionActuelle(),
				masses, C);
	}
